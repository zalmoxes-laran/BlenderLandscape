...
class EM_BasePanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.template_list(
            "EM_named_super_groups", "", scene, "super_groups", scene, "super_groups_index")
...
class EM_named_super_groups(UIList):
...
#        layout.operator(EM_super_group_add.bl_idname)
#        #layout.operator(EM_super_group_remove.bl_idname)
...
class EM_Add_Objects_Sub_Menu(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        for i, s_group in enumerate(context.scene.super_groups):
            op = layout.operator(EM_add_to_group.bl_idname, text=s_group.name)
            op.group_idx = i
...
class EM_Remove_SGroup_Sub_Menu(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        for i, s_group in enumerate(context.scene.super_groups):
            op = layout.operator(EM_super_group_remove.bl_idname, text=s_group.name)
            op.group_idx = i
...
class EM_Select_SGroup_Sub_Menu(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        for i, s_group in enumerate(context.scene.super_groups):
            op = layout.operator(EM_toggle_select.bl_idname, text=s_group.name)
            op.group_idx = i
            op.is_select = True
            op.is_menu = True
...
class EM_Toggle_Visible_SGroup_Sub_Menu(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        for i, s_group in enumerate(context.scene.super_groups):
            op = layout.operator(EM_toggle_visibility.bl_idname, text=s_group.name)
            op.group_idx = i
...
class EM_super_group_add(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        new_s_group.name = "EM.%.3d" % group_idx
        new_s_group.unique_id = uni_numb
        scene.super_groups_index = group_idx

        # add the unique id of selected objects
        for obj in context.selected_objects:
            EM_add_property_to_obj(new_s_group.unique_id, obj)

        return {'FINISHED'}

class EM_super_group_remove(bpy.types.Operator):
...
class EM_super_group_move(bpy.types.Operator):
        ...
        if self.group_idx < len(scene.super_groups):
            ...
            if s_group.use_toggle is True:
                if self.is_select is True:
                    ...
                    EM_select_objects(context, [s_group.unique_id], True)
                    if scene.sg_settings.unlock_obj:
                    ...
                    EM_select_objects(context, [s_group.unique_id], False)
        ...
        if scene_parse.super_groups:
            ...
            if s_group is not None and self.group_idx < len(scene_parse.super_groups):
                for obj in context.selected_objects:
                    ...
                    EM_add_property_to_obj(s_group.unique_id, obj)
...
class EM_remove_from_group(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if scene.super_groups:
            ...
            # remove s_groups
            for obj in context.selected_objects:
                EM_del_properties_from_obj(UNIQUE_ID_NAME, s_groups, obj, True)
            s_groups = None  # clear
...
def EM_del_properties_from_obj(prop_name, s_groups_ids, obj, delete_in_s_groups=True):
    props = obj.sg_belong_id
...
def register():
    ...
    bpy.types.Scene.super_groups = CollectionProperty(type=EM_Group)
    bpy.types.Object.sg_belong_id = CollectionProperty(type=EM_Object_Id)
